...
def test_detector_engine_backend_raises_when_runtime_fails(tmp_path) -> None:
    class BadRuntime:
        def run(self, input_batch: np.ndarray) -> np.ndarray:
            del input_batch
            raise RuntimeError("cuda unavailable")
...
def test_detector_engine_init_failure_is_cached_and_raises(tmp_path, monkeypatch) -> None:
    engine_path = tmp_path / "yolov8s.engine"
    pt_model_path = tmp_path / "best.pt"
    engine_path.write_bytes(b"fake-engine")
    pt_model_path.write_bytes(b"fake-weights")
    ...
    def fake_tensorrt_runner(*args, **kwargs):
        del args, kwargs
        nonlocal attempts
        attempts += 1
        raise RuntimeError("bad engine")

    monkeypatch.setattr("src.perception.detector.TensorRTEngineRunner", fake_tensorrt_runner)

    frame = np.zeros((480, 640, 3), dtype=np.uint8)
    with pytest.raises(TensorRTInferenceUnavailableError, match="bad engine"):
        detector.detect(frame, frame_id=0)
    with pytest.raises(TensorRTInferenceUnavailableError, match="bad engine"):
        detector.detect(frame, frame_id=1)
