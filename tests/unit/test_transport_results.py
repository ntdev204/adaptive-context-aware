...
def test_perception_result_roundtrip_uses_protobuf_bytes() -> None:
    message = PerceptionResultMessage(
        ...
        entities=[
            TrackedEntityMessage(
                ...
                contour_xy=np.array([[10.0, 20.0], [40.0, 20.0], [40.0, 60.0], [10.0, 60.0]], dtype=np.float32),
                contour_points_xyz_m=np.array(
                    [[1.0, 2.0, 3.0], [1.2, 2.0, 3.0], [1.2, 2.4, 3.0], [1.0, 2.4, 3.0]], dtype=np.float32
                ),
    ...
    assert decoded.entities[0].class_id == 0.0
    assert decoded.entities[0].distance_source == "depth_lidar_fused"
    assert decoded.entities[0].distance_to_robot_m == 2.5
    np.testing.assert_allclose(decoded.entities[0].position_xyz_m, np.array([1.0, 2.0, 3.0], dtype=np.float32))
    assert decoded.entities[0].contour_xy.shape == (4, 2)
